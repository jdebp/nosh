...
## **************************************************************************
#
# Special setup for mariadb.
# This is invoked by all.do .
#

# These get us *only* the configuration variables, safely.
read_rc() { clearenv read-conf rc.conf "`which printenv`" "$1" ; }
get_var() { read_rc mariadb_"$1"_"$2" || read_rc mariadb_"$2" || true ; }
...
list_instances() { test -e "${etcdir}"/my.cnf && echo "" ; test -d "${defaults_dir}" && for i in "${defaults_dir}"/my*.cnf ; do basename "$i" .cnf | sed -e 's/^my//' ; done ; }

redo-ifchange rc.conf general-services "mariadb@.service"

for etcdir in "/etc" "/etc/mysql" "/usr/local/etc" "/usr/local/etc/mysql" ""
do
	test -n "${etcdir}" || exit 0
	defaults_dir="${etcdir}/my.cnf.d"
	...
	if ! test -e "${defaults_dir}"
	then
		redo-ifcreate "${defaults_dir}"
		echo >>"$3" "${defaults_dir}" "does not exist."
		continue
	fi
	if ! test -d "${defaults_dir}"
	then
		redo-ifchange "${defaults_dir}"
		echo >>"$3" "${defaults_dir}" "is not valid."
		continue
	fi
	break
done

redo-ifchange "${etcdir}" "${defaults_dir}"
...
do
	...
	defaults_file="${etcdir}/${i:+my.cnf.d/}my$i.cnf"
