...
main (
	...
	if (0 > ok_fd) {
		...
		if (0 > ok_fd) return EXIT_TEMPORARY_FAILURE;
	}
	const int status_fd(open_read_at(dir_fd, "status"));
	if (0 > status_fd) return EXIT_TEMPORARY_FAILURE;
	char status[20];
	const int n(read(status_fd, status, sizeof status));
	if (0 > n) return EXIT_TEMPORARY_FAILURE;
	if (18 <= n) {
		if (20 <= n) {
			if (encore_status_running == status[18]) return EXIT_SUCCESS;
		} else {
			if (status[12] || status[13] || status[14] || status[15]) return EXIT_SUCCESS;
		}
	}
